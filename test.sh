...
for i in 1 2 ; do docker inspect internal-${INTERNAL}${i}_c1_1 > ${OUTDIR}/docker-inspect_${i}; done
for i in 1 2 ; do docker network inspect internal-${INTERNAL}${i}_default; done > ${OUTDIR}/docker-net-inspect
for i in 1 2 ; do docker-compose -f internal-${INTERNAL}.yml -p  internal-${INTERNAL}${i} config > $OUTDIR/compose-config_${i}; done
